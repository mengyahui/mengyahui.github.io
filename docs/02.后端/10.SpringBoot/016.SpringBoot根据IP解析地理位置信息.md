...
Ip2region 为每个 ip 数据段的 region 信息都固定了格式：`国家|区域|省份|城市|ISP`，只有中国的数据绝大部分精确到了城市，其他国家部分数据只能定位到国家，后前的选项全部是 0。
...
和 Ip2region 一样 GeoLite2 也是一个离线 IP 定位数据库。GeoLite2 不仅可以获取城市级别的位置信息，还可以获取经纬度。对于国内的 IP 查询的经度比不上 Ip2region，此外返回的经纬度也不是很准确。
